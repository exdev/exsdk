...
// File         : CRC.c
// Author       : Wu Jie 
// Last Change  : 06/28/2010 | 11:53:46 AM | Monday,June
// Description  : 
// ======================================================================================
...
///////////////////////////////////////////////////////////////////////////////
    ...
    // CCITT Standard Polynomial: x^32+x^26+x^23+x^22+x^16+x^12+x^11+x^10+x^8+x^7+x^5+x^4+x^2+x+1.

    // NOTE: the disadvantage to use crc, is you add more 1-kbytes long table 
    // NOTE: the disadvantage to use fast crc, is you add more 4-kbytes long table 

    // ------------------------------------------------------------------ 
    // Desc: CRC_Table0 
    // ------------------------------------------------------------------ 

    static const uint32 CRC_Table0[256] =
    {
        ...
        static const uint32 CRC_Table1[256] =
        {
        ...
        static const uint32 CRC_Table2[256] =
        {
        ...
        static const uint32 CRC_Table3[256] =
        {
...
// ======================================================== 
    ...
    if (_len && ((size_t)_data & 2))
    {            
        result = CRC_Table0[(result ^ *_data++) & 0xff] ^ (result >> 8);
        result = CRC_Table0[(result ^ *_data++) & 0xff] ^ (result >> 8);
        _len -= 2;
    }

    if (_len && ((size_t)_data & 1))
    {
        result = CRC_Table0[(result ^ *_data++) & 0xff] ^ (result >> 8);
        _len--;
    }
