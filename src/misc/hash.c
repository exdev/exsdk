...
// File         : hash.c
// Author       : Wu Jie 
// Last Change  : 06/28/2010 | 11:18:20 AM | Monday,June
// Description  : 
// ======================================================================================
...
// ------------------------------------------------------------------
// Desc: 
//  hash a variable-length _data into a 32-bit value
//      @params
//        _data     : the _data (the unaligned variable-length array of bytes)
//        _len       : the length of the _data, counting by bytes
//        _seed      : can be any 4-byte value
//      Returns a 32-bit value.  Every bit of the _data affects every bit of
...
uint32 ex_hashbob2( const uint8* _data, uint32 _len, uint32 _seed )
{
    ...
    /*---------------------------------------- handle most of the _data */
    while (len >= 12)
    {
        a += (_data[0] +((uint32)_data[1] << 8) +((uint32)_data[2]  << 16) +((uint32)_data[3] << 24));
        b += (_data[4] +((uint32)_data[5] << 8) +((uint32)_data[6]  << 16) +((uint32)_data[7] << 24));
        c += (_data[8] +((uint32)_data[9] << 8) +((uint32)_data[10] << 16)+((uint32)_data[11] << 24));
        __mix(a,b,c);
        _data += 12; len -= 12;
    }
    ...
    {
    case 11: c+=((uint32)_data[10]<<24);
    case 10: c+=((uint32)_data[9]<<16);
    case 9 : c+=((uint32)_data[8]<<8);
             /* the first byte of c is reserved for the length */
    case 8 : b+=((uint32)_data[7]<<24);
    case 7 : b+=((uint32)_data[6]<<16);
    case 6 : b+=((uint32)_data[5]<<8);
    case 5 : b+=_data[4];
    case 4 : a+=((uint32)_data[3]<<24);
    case 3 : a+=((uint32)_data[2]<<16);
    case 2 : a+=((uint32)_data[1]<<8);
    case 1 : a+=_data[0];
...
//      _data     : the _data, an array of uint32 values
//      _len      : the length of the _data, counting by bytes
//      _seed1    :    IN: seed OUT: primary hash value 
...
void ex_hashword2 ( const uint32 *_data, uint32 _len, uint32 *_seed1, uint32 *_seed2 )
{
    ...
    /* Set up the internal state */
    a = b = c = 0xdeadbeef + ((uint32)(_len<<2)) + *_seed1;
    c += *_seed2;
...
uint32 ex_hashlittle ( const void *_data, uint32 _len, uint32 _seed )
{
    ...
    {                        /* need to read the _data one byte at a time */
        const uint8 *pData8 = (const uint8 *)_data;
...
// the _data.  *_seed1 is better mixed than *_seed2, so use *_seed1 first.  If you want
// a 64-bit value do something like "*_seed1 + (((uint64_t)*_seed2)<<32)".
//
//  @params:
//      _data     : the _data to hash
//      _len      : the length of the _data, counting by bytes
//      _seed1    :    IN: seed OUT: primary hash value 
//      _seed2    :    IN: more seed OUT: secondary hash value 
// ------------------------------------------------------------------
...
void ex_hashlittle2 ( const void *_data, uint32 _len, uint32 *_seed1, uint32 *_seed2 ) 
{
    ...
    /* Set up the internal state */
    a = b = c = 0xdeadbeef + ((uint32)_len) + *_seed1;
    c += *_seed2;
    ...
    if (__HASH_LITTLE_ENDIAN && ((u.i & 0x3) == 0)) 
    {
        ...
        case 2 : a+=pData32[0]&0xffff;                                      break;
        case 1 : a+=pData32[0]&0xff;                                        break;
        case 0 : *_seed1=c; *_seed2=b; return;  /* zero _len strings require no mixing */
        }
...
void ex_hashbig2( const void *_data, uint32 _len, uint32 *_seed1, uint32 *_seed2 )
{
    ...
    if (__HASH_BIG_ENDIAN && ((u.i & 0x3) == 0)) 
    {
        ...
        case 2 : a+=pData32[0]&0xffff0000; break;
        case 1 : a+=pData32[0]&0xff000000; break;
        case 0 : *_seed1=c; *_seed2=b; return;  /* zero length strings require no mixing */
        }
...
uint32 ex_hashpaul( const uint8 *_data, uint32 _len )
{
    ...
    for ( ; _len > 0; --_len ) 
    {
        hash    += __get_uint16(_data);
        tmp      = (__get_uint16(_data+2) << 11) ^ hash;
        hash     = (hash << 16) ^ tmp;
        _data  += 2*sizeof(uint16);
        hash    += hash >> 11;
    }
    ...
    switch (rem)
    {
    case 3: hash += __get_uint16(_data);
            hash ^= hash << 16;
            hash ^= _data[sizeof(uint16)] << 18;
            hash += hash >> 11;
            break;
    case 2: hash += __get_uint16(_data);
            hash ^= hash << 11;
            hash += hash >> 17;
            break;
    case 1: hash += *_data;
