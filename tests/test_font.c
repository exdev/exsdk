...
// File         : test_font.c
// Author       : Wu Jie 
// Last Change  : 01/09/2013 | 21:49:28 PM | Wednesday,January
// Description  : 
// ======================================================================================
...
#include "exsdk.h"
#include "gl/font/vertex_buffer.h"
#include "gl/font/markup.h"
...
console_render( console_t *self )
{
    ...
    {
        wchar_t *text = * (wchar_t **) ex_array_get( self->lines, i ) ;
        if( wcslen(text) > 0 )
        {
...
console_print( console_t *self, wchar_t *text )
{
    ...
    if( end != NULL)
    {
        ...
        ex_array_set( self->lines, self->lines->count-1, &line );
        free( last_line );
